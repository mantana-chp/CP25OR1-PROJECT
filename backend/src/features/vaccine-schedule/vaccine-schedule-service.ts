...
export const calculateVaccineSchedule = async (input: CalculateScheduleInput): Promise<Appointment[]> => {
  ...
  if (!pet.birth_date) {
    throw new ApiError('Pet birth date is required for schedule calculation.', 400);
  }
  ...
  if (vaccine.primary_series_logic === VaccineLogicType.FIXED_COUNT) {
    ...
    // Puppy/Kitten Case
      ...
      while (differenceInDays(currentDate, birthDate) < vaccine.primary_target_value) {
        schedule.push({
          doseNumber: doseCounter++,
          date: format(currentDate, 'yyyy-MM-dd'),
          type: 'Primary',
  ...
  if (lastPrimaryDoseDate) {
    // Booster 1
    const booster1Date = addDays(lastPrimaryDoseDate, vaccine.booster_1_interval_days);
    schedule.push({
      doseNumber: doseCounter++,
    ...
    const nextBoosterDate = addDays(booster1Date, vaccine.booster_repeat_interval_days);
    schedule.push({
      doseNumber: doseCounter++,
