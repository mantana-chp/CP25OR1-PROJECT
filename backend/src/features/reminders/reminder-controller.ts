...
export const createMultipleReminders = asyncHandler(
  async (req: Request, res: Response) => {
    const validatedData = createMultipleRemindersSchema.parse(req).body
    const { id: userId } = req.user!
    const result = await reminderService.createMultipleReminders(
      validatedData,
      userId,
    )
...
export const updateReminder = asyncHandler(
  async (req: Request, res: Response) => {
    const {
      ...
      body: validatedData,
    }: { params: { id: string }; body: UpdateReminderPayload } =
      updateReminderSchema.parse(req)
    const { id: userId } = req.user!
...
export const toggleReminderStatus = asyncHandler(
  async (req: Request, res: Response) => {
    const { id: reminderId } = getReminderByIdSchema.parse(req.params)
    const { id: userId } = req.user!
    const updatedReminder = await reminderService.toggleReminderStatus(
      reminderId,
      userId,
    )
    sendSuccess(res, updatedReminder)
