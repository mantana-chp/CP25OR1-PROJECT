...
import { CreateReminderPayload, UpdateReminderPayload, DeleteReminderQuery } from './reminder-schema';
      ...
      if (rule.daysOfWeek) {
        for (let i = 1; i <= 30; i++) {
          ...
          if ((rule.daysOfWeek & dayBit) > 0) {
            if (rule.interval > 1) {
              const diffDays = Math.ceil((checkDate.getTime() - lastDate.getTime()) / (1000 * 3600 * 24));
              if (Math.floor(diffDays / 7) % rule.interval === 0) {
                return checkDate;
              }
            } else {
              return checkDate;
...
const generateNextInstance = async (tx: Prisma.TransactionClient, completedReminder: reminders & { recurrence: recurrence | null; recurring_template: (reminders & { recurrence: recurrence | null }) | null }) => {
  const template = completedReminder.recurring_template ?? completedReminder;
  const rule = template.recurrence;

  if (!rule) return;

  const nextDate = calculateNextOccurrence(completedReminder.reminder_date, rule);
  if (!nextDate) return;
  ...
  if (rule.endAfterOccurrences) {
    ...
    // The current reminder's status is updated within the same transaction, so we optimistically add 1
    if ((instanceCount + 1) >= rule.endAfterOccurrences) {
      return;
    }
  }

  const existingNextInstance = await tx.reminders.findFirst({
    where: {
      OR: [
        { id: template.id },
...
export const deleteReminder = async (id: string, userId: string, deleteScope?: 'THIS_INSTANCE_ONLY' | 'ALL_INSTANCES'): Promise<void> => {
  ...
  if (isRecurring && scope === 'ALL_INSTANCES') {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 2. Delete any future, un-done instances that were already created
      ...
      // 3. If the template itself is still to-do, delete it as well,
      // as it's an upcoming event that is now cancelled.
      // This is safe because it has no children left to cascade to.
      if (template.reminder_status !== 'done') {
  ...
  await prisma.$transaction(async (tx) => {
    // If it's a recurring instance being skipped, generate the next one to keep the chain alive
    if (isRecurring) {
      await generateNextInstance(tx, reminder);
    }
    // Standard deletion of the single reminder
