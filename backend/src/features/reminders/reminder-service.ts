  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (reminderToToggle.parent_id) {
      ...
      const allChildrenDone = siblings.every(
        (r) => r.reminder_status === reminder_status.done,
      )
...
export const updateReminder = async (
  ...
  if (existingReminder.reminder_status === reminder_status.done) {
    throw new BadRequestError(
      'Cannot edit a reminder that is marked as "done".',
    )
  }
  ...
  const newReminderName =
    updateData.reminderName ?? existingReminder.reminder_name
  const newReminderDate = updateData.reminderDate
  ...
  if (updateData.reminderName || updateData.reminderDate) {
    ...
    if (conflictingReminder) {
      throw new ConflictError(
        'A reminder with this name and date already exists for the selected pet.',
      )
    }
  }
  ...
  const newStatus = isReminderOverdue(tempReminder, new Date())
    ? reminder_status.overdue
    : reminder_status.to_do
  ...
  if (updateData.reminderTime !== undefined) {
    dataToUpdate.reminder_time = updateData.reminderTime
      ? new Date(`1970-01-01T${updateData.reminderTime}Z`)
      : null
  }
  if (updateData.categoryName !== undefined) {
    if (
      existingReminder.parent_id != null &&
      existingReminder.category_name === category_name.Vaccination
    ) {
      // child reminder (vaccine) cant change category
      throw new ConflictError(
        'Cannot change the category of a vaccination reminder.',
      )
    }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (updateData.children && updateData.children.length > 0) {
      for (const child of updateData.children) {
        ...
        const childStatus = isReminderOverdue(childTempReminder, new Date())
          ? reminder_status.overdue
          : reminder_status.to_do
