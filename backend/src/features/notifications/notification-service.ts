...
import { expoPushService, PushMessage, ExpoPushTicket } from '../../services/expo-push-service';
...
export const processAndSendNotifications = async () => {
  ...
  if (unnotifiedReminders.length > 0) {
    logger.info(`[NotificationJob] Found ${unnotifiedReminders.length} reminders to process.`);
  }
  ...
  for (const reminder of unnotifiedReminders) {
    ...
    if (reminder.reminder_time) {
      ...
      const reminderDateTime = new Date(`${datePart}T${timePart.replace('Z', '+07:00')}`);
      notificationSendTime = new Date(reminderDateTime.getTime() - 30 * 60 * 1000);
    } else {
    ...
    if (notificationSendTime <= now) {
      const newNotification = await createNotificationAndPreparePush(reminder, messagesToSend);
      if (newNotification && reminder.user.push_tokens.length > 0) {
        ...
        notificationIdsToUpdate[reminder.user.push_tokens[0].token] = newNotification.id;
      }
    }
  }
  ...
  if (messagesToSend.length > 0) {
    logger.info(`[NotificationJob] Sending ${messagesToSend.length} push notifications via Expo service.`);
    ...
    for (const ticket of tickets) {
      ...
      const notificationId = notificationIdsToUpdate[messagesToSend.find(m => m.to === ticket.id)?.to || '']; // This mapping is flawed, needs to be by reminder_id or a unique message ID
      ...
      if (notificationId) {
        if (ticket.status === 'ok') {
          ...
          logger.info(`[NotificationJob] Notification ${notificationId} status updated to 'sent'.`);
        } else {
          await notificationRepository.update(notificationId, {
            status: notification_status.failed,
          });
          logger.error(`[NotificationJob] Notification ${notificationId} status updated to 'failed': ${ticket.message}`);
        }
      }
    }
  }
 ...
 */
async function createNotificationAndPreparePush(reminder: any, messagesToSend: PushMessage[]) {
  ...
  logger.info(`[NotificationJob] Created in-app notification for reminder: ${reminder.reminder_name}`);

  if (reminder.user.push_tokens.length > 0) {
    for (const token of reminder.user.push_tokens) {
      logger.info(`[NotificationJob] Preparing push notification for token: ${token.token}`);
      messagesToSend.push({
        to: token.token,
