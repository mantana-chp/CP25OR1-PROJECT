...
import { expoPushService, PushMessage } from '../../services/expo-push-service'
...
export const getNotifications = async (
  ...
  const notificationsFromDb: NotificationWithRelations[] =
    await notificationRepository.findManyByUserId(userId, isRead)
...
export const processAndSendNotifications = async () => {
  ...
  if (candidateReminders.length === 0) {
    logger.info(
      '[NotificationJob] No unnotified candidate reminders found for today/tomorrow.'
    )
  ...
  logger.info(
    `[NotificationJob] Found ${candidateReminders.length} candidate reminders for today/tomorrow: [${candidateIds.join(', ')}]`
  )
  ...
  const dueReminders = candidateReminders.filter((reminder) => {
    ...
    if (reminder.reminder_time) {
      ...
      const bangkokTimeMillis =
        reminder.reminder_time.getTime() -
      ...
      notificationSendTimeUTC = new Date(
        reminderActualTimeUTC.getTime() - 30 * 60 * 1000
      )
    } else {
      ...
      reminderActualTimeUTC = new Date(
        Date.UTC(
          ...
          16,
          59,
          59 // 16:59:59 UTC = 23:59:59 Bangkok (same day)
        )
      )
    }
    ...
    const withinGracePeriod =
      now <= new Date(reminderActualTimeUTC.getTime() + gracePeriodMs)
  ...
  if (dueReminders.length === 0) {
    logger.info(
      '[NotificationJob] No reminders are currently due for processing after time-based filtering.'
    )
    logger.info('--- FINISHED NOTIFICATION JOB ---')
    return
  }
  ...
  logger.info(
    `[NotificationJob] Found ${dueReminders.length} reminders that are actually due for notification: [${dueReminders.map((r) => r.id).join(', ')}]`
  )
  ...
  for (const reminder of dueReminders) {
    ...
    const recipients = [
      ...
      ...caregiverAccess.map((a) => ({
        userId: a.user_id,
        pushTokens: a.user.push_tokens
      }))
    ]

    for (const recipient of recipients) {
      // Per-user: find this recipient's existing notification for this reminder
      const userNotifications = reminder.notifications.filter(
      ...
      if (retryCount >= MAX_RETRY_ATTEMPTS) {
        logger.warn(
          `[NotificationJob] Skipping user ${recipient.userId} for reminder ${reminder.id} (max retries: ${retryCount}/${MAX_RETRY_ATTEMPTS}).`
        )
        continue
      }
      ...
      if (isRetry && failedNotification) {
        const timeSinceLastAttempt =
          now.getTime() - new Date(failedNotification.created_at!).getTime()
        if (timeSinceLastAttempt < RETRY_INTERVAL_MS) {
          continue // Too soon to retry
        }
        logger.info(
          `[NotificationJob] Retry attempt #${retryCount + 1} for user ${recipient.userId}, reminder ${reminder.id}.`
        )
      }
      ...
      if (!petName || !reminderName) {
        ...
        if (failedNotification) {
          await notificationRepository.update(failedNotification.id, {
      ...
      if (isRetry && failedNotification) {
        ...
        logger.info(
          `[NotificationJob] Reusing existing notification ${notificationToProcess.id} for retry.`
        )
        ...
        for (const token of recipient.pushTokens) {
          messagesToSend.push({
            ...
            data: {
              ...
              notificationId: notificationToProcess.id
            }
          })
        }
      } else {
        logger.info(
          `[NotificationJob] No push tokens for user ${recipient.userId}. Notification ${notificationToProcess.id} saved for in-app only.`
        )
      }
      ...
      if (messagesToSend.length > 0) {
        try {
          logger.info(
            `[NotificationJob] Sending ${messagesToSend.length} push notifications for notification ID: ${notificationToProcess.id}${isRetry ? ` (retry #${retryCount + 1})` : ''}`
          )
          await expoPushService.send(messagesToSend)
          logger.info(
            `[NotificationJob] Successfully sent push notifications for ${notificationToProcess.id}.`
          )
        } catch (error: unknown) {
          logger.error(
          ...
          if (retryCount + 1 >= MAX_RETRY_ATTEMPTS - 1) {
            logger.warn(
              `[NotificationJob] ⚠️ Reminder ${reminder.id} / user ${recipient.userId} has failed ${retryCount + 1} times.`
            )
          }
        }
      }
      ...
      logger.info(
        `[NotificationJob] Marked notification ${notificationToProcess.id} as ${finalStatus}.`
      )
    }
  }
...
export const sendTipNotification = async (
  ...
  logger.info(
    `[TipNotification] Preparing AI tip notification for user ${userId}, pet ${petId}.`
  )
  let finalStatus: notification_status = notification_status.sent
  let sentAt: Date | null = new Date()
  ...
  try {
    ...
    const hasPushTokens =
      userWithTokens &&
      userWithTokens.push_tokens &&
      userWithTokens.push_tokens.length > 0
    const messagesToSend: PushMessage[] = []
    ...
    if (hasPushTokens) {
      ...
      logger.info(
        `[TipNotification] Notification ${newNotificationId} will be processed for in-app only (no push tokens).`
      )
    }
    ...
    if (messagesToSend.length > 0) {
      try {
        logger.info(
          `[TipNotification] Sending ${messagesToSend.length} push notifications for notification ID: ${newNotificationId}`
        )
        await expoPushService.send(messagesToSend)
        logger.info(
          `[TipNotification] Push notifications sent successfully for ${newNotificationId}.`
        )
      } catch (error: unknown) {
        if (error instanceof Error) {
          logger.error(
            `[TipNotification] Failed to send push for notification ${newNotificationId}:`,
            error
          )
        } else {
          logger.error(
    ...
    logger.error(
      `[TipNotification] Error processing AI tip notification for user ${userId}, pet ${petId}:`,
      error as Error
    )
    finalStatus = notification_status.failed
    sentAt = null
    ...
    logger.info(
      `[TipNotification] Marked notification ${newNotificationId} as ${finalStatus}.`
    )
  }
}
...
export const sendStatusChangeNotification = async (
  ...
  try {
    ...
    if (recipients.length === 0) {
      logger.info(
        `[StatusChangeNotification] No other recipients for reminder ${reminderId}.`
      )
      return
    }
    ...
    for (const recipient of recipients) {
      ...
      if (recipient.pushTokens.length > 0) {
        ...
        try {
          ...
          logger.info(
            `[StatusChangeNotification] Push sent to user ${recipient.userId} for reminder ${reminderId}.`
          )
        } catch (error: unknown) {
          logger.error(
        ...
        logger.info(
          `[StatusChangeNotification] No push tokens for user ${recipient.userId}. In-app only.`
        )
      }
    ...
    logger.error(
      '[StatusChangeNotification] Unexpected error:',
      error instanceof Error ? error : new Error(String(error))
    )
  }
}
...
export const sendHealthInsightNotification = async (
  ...
  try {
    ...
    logger.info(
      `[HealthInsightNotification] Sending to ${recipients.length} recipients (owner + ${caregiverAccess.length} caregivers)`
    )
    ...
    for (const recipient of recipients) {
      ...
      if (recipient.pushTokens && recipient.pushTokens.length > 0) {
        ...
        logger.info(
          `[HealthInsightNotification] No push tokens for user ${recipient.userId}. Notification ${notification.id} saved for in-app only.`
        )
      }
      ...
      if (messagesToSend.length > 0) {
        try {
          logger.info(
            `[HealthInsightNotification] Sending ${messagesToSend.length} push notifications for notification ID: ${notification.id}`
          )
          await expoPushService.send(messagesToSend)
          logger.info(
            `[HealthInsightNotification] Push notifications sent successfully for ${notification.id}.`
          )
        } catch (error: unknown) {
          logger.error(
      ...
      logger.info(
        `[HealthInsightNotification] Marked notification ${notification.id} as ${finalStatus}.`
      )
    }

    logger.info(
      `[HealthInsightNotification] Completed notification delivery for insight ${insightId}`
    )
  } catch (error) {
    logger.error(
      '[HealthInsightNotification] Unexpected error:',
      error instanceof Error ? error : new Error(String(error))
    )
  }
}
