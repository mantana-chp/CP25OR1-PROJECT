...
export const listAccessListController = asyncHandler(
  async (req: Request, res: Response) => {
    const { params } = listAccessListSchema.parse(req)
    const { id: userId } = req.user!
    const result = await service.listAccessList(
      params.petId,
      userId,
      req.petRole,
    )
...
export const revokeCaregiverController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const { id: userId } = req.user!
    const result = await service.revokeCaregiver(
      params.petId,
      params.accessId,
      userId,
    )
