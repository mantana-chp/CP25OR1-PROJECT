...
import { analyzeAllPetsAndSendInsights } from '../features/health-insights/health-insight-orchestrator';
...
// Schedule: 0 5 * * * = 05:00 UTC = 12:00 Asia/Bangkok (GMT+7)
// Server timezone: Asia/Bangkok (+07)
const aiTipNotificationJob = cron.schedule('0 5 * * *', async () => {
  const now = new Date();
...
// New job for analyzing health logs and sending personalized health insights
// IMPORTANT: Cron schedule uses UTC time (explicitly set with timezone option)
// Schedule: 0 12 * * * = 12:00 UTC = 19:00 Asia/Bangkok (GMT+7)
// Runs 7 hours after AI tips (7 hours between 12:00 and 19:00)
const healthInsightJob = cron.schedule('0 12 * * *', async () => {
  const now = new Date();
  const bangkokTime = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Bangkok' }));
  logger.info(`Running job: Analyzing health logs and sending insights...`);
  logger.info(`Job triggered at: ${now.toISOString()} UTC | Bangkok time: ${bangkokTime.toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' })}`);
  try {
    await analyzeAllPetsAndSendInsights();
  } catch (error: unknown) {
    if (error instanceof Error) {
      logger.error('Error running health insights job:', error);
    } else {
      logger.error('Error running health insights job:', new Error(String(error)));
    }
  }
}, {
  timezone: 'Etc/UTC' // Explicitly use UTC timezone
...
export const startNotificationScheduler = () => {
  ...
  logger.info('   Schedule: 0 5 * * * (05:00 UTC daily)');
  logger.info('   Bangkok time: 12:00 (12:00 PM / noon) daily');
  logger.info('   Description: Generates and sends personalized AI tips');
  logger.info('');
  logger.info('📌 Health Insights Notifications:');
  logger.info('   Schedule: 0 12 * * * (12:00 UTC daily)');
  logger.info('   Bangkok time: 19:00 (7:00 PM) daily');
  logger.info('   Description: Analyzes health logs and sends personalized health insights');
  logger.info('========================================');
};
